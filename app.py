...
with st.form("house_features_form"):
    ...
    with col1:
        area = st.slider("Area (in sqft)", min_value=1500, max_value=17000, value=5000, step=100)
        ...
        price_per_sqft = st.number_input("Price per sqft (Scaled: Price/1000/Area)", min_value=0.40, max_value=3.00, value=1.00, step=0.01)
    ...
    if submitted:
        ...
        try:
            ...
            st.info("Note: The model predicts the price in thousands (₹'000) internally, then scaled back for display.")
